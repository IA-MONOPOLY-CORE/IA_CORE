...
- **Presets por dominio — COMPLETADO:** `domains/loteria/agent_presets.json` crea la primera biblioteca semilla de presets operativos para combinaciones `role_id + specialization_id`; `core/domain_registry.py` la valida contra `profile_catalog.json` y `api.py` la expone por endpoints read-only sin conectar todavía Crear Agente.
...
| core/domain_registry.py | NO | Registro y resolución genérica de dominios: lee `domain.json`, crea dominios, resuelve carpetas de agentes/config/papers/memory_sources por `domain_id`, carga/valida `domains/<domain_id>/profile_catalog.json`, carga/valida `domains/<domain_id>/agent_presets.json` contra el perfil del dominio y detecta ambigüedad de IDs de agente entre dominios |
...
| domains/loteria/agent_presets.json | SÍ | Biblioteca semilla de presets operativos de agentes para Lotería: define combinaciones `role_id + specialization_id`, nombre/id sugeridos, descripción, system prompt, criterios, sesgos a evitar, política de memoria y `paper_seed`; todavía no es consumida por Crear Agente ni genera papers automáticamente |
