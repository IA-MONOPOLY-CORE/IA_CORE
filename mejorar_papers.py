...
def mejorar_paper(agente_id: str, usar_llm: bool = True):
    ...
    print(
        f"   📊 Desde JSON: {len(lecciones_json.get('reglas_clave', []))} reglas, {len(lecciones_json.get('errores_a_evitar', []))} errores"
    )
    ...
    if usar_llm:
        ...
        if llm:
            ...
            if lecciones_vectorial:
                print(
                    f"   🤖 Desde vectorial: {len(lecciones_vectorial.get('reglas_clave', []))} reglas, {len(lecciones_vectorial.get('lecciones_aprendidas', []))} lecciones"
                )
        else:
