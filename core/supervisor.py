...
class Supervisor:
    ...
    def __init__(
        ...
        *,
        ...
        self._evolution_manager_class = (
            evolution_manager_class or self._get_default_evolution_manager_class()
        )
        self._score_response_fn = score_response_fn or self._get_default_score_response_fn()
        self._build_scores_summary_fn = (
            build_scores_summary_fn or self._get_default_build_scores_summary_fn()
        )
    ...
    def orchestrate(
        ...
        *,
        ...
        return asyncio.run(self.orchestrate_async(task, agent_names, mode=mode))
    ...
    async def orchestrate_async(
        ...
        *,
        ...
        if mode is ExecutionMode.SEQUENTIAL:
            ...
            debate = await self._run_debate_async(
                execution_id, task, targets, deadline=orch_deadline
            )
            ...
            result.success = (
                any(step.success for step in debate.steps)
                if continue_on_failure
                else all(step.success for step in debate.steps)
            )
    ...
    async def _run_debate_async(
        ...
        for round_idx in range(1, TOTAL_RECURSIVE_ROUNDS + 1):
            if deadline is not None and time.perf_counter() > deadline:
                logger.error(
                    "Debate %s abortado por timeout global en ronda %d", debate_id, round_idx
                )
                break

            logger.info(
                f"🔄 [Mesa Redonda Cuántica - Ronda {round_idx}/{TOTAL_RECURSIVE_ROUNDS}] Lanzando {len(agent_roles)} agentes en paralelo..."
            )
            ...
            if round_idx == 1:
                ...
                resumenes_previos = [
                    f"● Resumen Ronda {i + 1}: {r['summary']}"
                    for i, r in enumerate(historial_por_ronda)
                ]
            ...
            try:
                ...
                logger.error(
                    f"⚠️ Alerta: Uno o más agentes superaron el timeout de {agent_timeout}s en la ronda {round_idx}"
                )
            ...
            for idx, res in enumerate(resultados_paralelos):
                ...
                if isinstance(res, asyncio.TimeoutError) or res is None:
                    logger.warning(
                        f"⏳ TIMEOUT AGENTE: {real_agent_id} (rol: {role_name}) en ronda {round_idx}/{TOTAL_RECURSIVE_ROUNDS} - superó {agent_timeout}s"
                    )
                    continue
                if isinstance(res, Exception) or not res.success:
                    logger.error(
                        f"Fallo crítico en agente {real_agent_id} durante ronda {round_idx}: {res}"
                    )
                ...
                bloque_respuestas_ronda.append(
                    f"  ▶️ [{role_name.upper()} - {real_agent_id}]: {text_clean}"
                )
            ...
            logger.info(
                f"🧠 Optimizando memoria semántica: Resumiendo colisiones de la ronda {round_idx}..."
            )
            resumen_ronda = await self._resumir_bloque_async(registro_ronda, round_idx)

            historial_por_ronda.append({"raw": registro_ronda, "summary": resumen_ronda})
            ...
            if round_idx >= 2:
                ...
                )  # convertir a 0-100 para comparar con agreement_score
        ...
        resumenes_totales = [
            f"● Ronda {i + 1}: {r['summary']}" for i, r in enumerate(historial_por_ronda)
        ]
    ...
    async def _execute_single_quantum_agent_async(
        ...
        if not agent:
            return AgentStepResult(
                ...
                success=False,
                error=f"No agent found for ID {agent_id_to_use}",
            )
        ...
        try:
            ...
            if hasattr(agent, "system_prompt"):
                response = await asyncio.to_thread(
                    agent.run, prompt_final, system_prompt=system_prompt
                )
            else:
            ...
            combinacion = (
                [int(n) for n in numeros_encontrados[:6]] if len(numeros_encontrados) >= 6 else None
            )
            ...
            return AgentStepResult(
                ...
                result={
                    ...
                    "llm": response.get("llm", {}) if isinstance(response, dict) else {},
                },
                duration_ms=duration,
                score=score_data,
    ...
    async def _execute_debate_turn_async(
        ...
        parent_id: str | None,
        step_id: str,
        current_steps: list[AgentStepResult],
    ) -> AgentStepResult:
        ...
        logger.info(
            f"🔍 Buscando agente de cierre: {agent_id_to_use} | Parent ID vinculado: {parent_id}"
        )
        ...
        logger.info(
            f"✅ Agente encontrado: {agent.id} | role: {getattr(agent, 'role', 'Unknown')} | provider: {provider_name}"
        )
        ...
        try:
            ...
            try:
                ...
                if lecciones_externas:
                    logger.info(
                        f"💡 {agent.id}: {len(lecciones_externas)} lecciones útiles inyectadas al prompt"
                    )
            except Exception as e:
            ...
            if hasattr(agent, "system_prompt"):
                response = await asyncio.to_thread(
                    agent.run, prompt_base, system_prompt=system_prompt, context=context
                )
            else:
            ...
            logger.info(
                f"✅ Respuesta recibida de {agent.id} | longitud: {len(text_response)} chars"
            )
    ...
    def _run_sequential(
        self, execution_id: str, task: str, targets: list[str], deadline: float
    ...
    async def _registrar_aprendizaje_post_debate(self, debate_result: DebateResult) -> None:
        ...
        logger.info(
            f"🔵 contradiction_score={debate_result.contradiction_score}, agreement_score={debate_result.agreement_score}, final_response existe={bool(debate_result.final_response)}"
        )

        try:
            from core.memoria_perpetua import cargar_memoria, guardar_memoria
            from core.herramientas import (
                extraer_herramientas_de_respuesta,
                registrar_herramienta,
                buscar_lecciones_utiles,
            )
            ...
            if debate_result.contradiction_score > 0 and debate_result.final_response:
                ...
                leccion_compartida = {
                    ...
                    "leccion": synthesis_text[:500]
                    if synthesis_text
                    else "Sin síntesis disponible",
                    "agentes_participantes": [
                        step.agent_name for step in debate_result.steps if step.success
                    ],
                }
                ...
                logger.info(
                    f"📝 Contradicción resuelta registrada en memoria compartida (contradicción: {debate_result.contradiction_score}% → acuerdo: {debate_result.agreement_score}%)"
                )
            else:
                ...
                if fue_acierto:
                    ...
                    if score >= 70:
                        ...
                        for herramienta in herramientas:
                            ...
                            logger.info(
                                f"🔧 {agente_id}: Herramienta '{herramienta['nombre']}' registrada (score: {score})"
                            )
            ...
            if best_agent and best_score >= 70:
                try:
                    ...
                    threading.Thread(
                        target=mejorar_paper, args=(best_agent,), kwargs={"usar_llm": False}
                    ).start()
                    logger.info(
                        f"🔄 Regeneración automática de paper iniciada para {best_agent} (best score: {best_score})"
                    )
                except Exception as e:
            ...
            for step in debate_result.steps:
                if step.success and step.role:
                    try:
                        ...
                        if lecciones_utiles:
                            logger.info(
                                f"💡 {step.agent_name}: {len(lecciones_utiles)} lecciones útiles disponibles de otros agentes"
                            )
                    except Exception as e:
                        logger.warning(f"Error buscando lecciones útiles: {e}")
