"""Schema de execution runner contract sin ejecucion real."""
...
EXECUTION_RUNNER_CONTRACT_SCHEMA_VERSION = "1.0"
EXECUTION_RUNNER_CONTRACT_TYPE = "execution_runner_contract"
...
ALLOWED_EXECUTION_RUNNER_MODES = {
    "contract_only",
    "dry_run_only",
    "simulation_only",
...
BLOCKED_EXECUTION_RUNNER_MODES = ALLOWED_EXECUTION_RUNNER_MODES - {"contract_only"}
...
def build_execution_runner_contract_report(
    *,
    contract_id: str,
    mode: str,
    target_type: str,
    ...
    payload = {
        "schema_version": EXECUTION_RUNNER_CONTRACT_SCHEMA_VERSION,
        "contract_id": contract_id,
        "contract_type": EXECUTION_RUNNER_CONTRACT_TYPE,
        "version": "1.0",
        "mode": mode,
    ...
    return validate_execution_runner_contract_report(payload)


def validate_execution_runner_contract_report(report: dict[str, Any]) -> dict[str, Any]:
    if not isinstance(report, dict):
        raise ValueError("execution_runner_contract debe ser un objeto")
    missing = REQUIRED_FIELDS - set(report)
    if missing:
        raise ValueError(f"execution_runner_contract incompleto: {', '.join(sorted(missing))}")
    if report.get("schema_version") != EXECUTION_RUNNER_CONTRACT_SCHEMA_VERSION:
        raise ValueError("schema_version de execution_runner_contract invalida")
    if report.get("contract_type") != EXECUTION_RUNNER_CONTRACT_TYPE:
        raise ValueError("contract_type de execution_runner_contract invalido")
    _validate_id(report.get("contract_id"), "contract_id")
    if report.get("mode") not in ALLOWED_EXECUTION_RUNNER_MODES:
...
def _ensure_json_serializable(payload: dict[str, Any]) -> None:
    try:
        ...
        raise ValueError("execution_runner_contract debe ser serializable como JSON") from exc
