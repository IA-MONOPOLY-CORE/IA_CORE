...
async def _run_validation_debate(validation_id: str, sorteo: int, task: str) -> None:
    ...
    validation_store[validation_id]["fase"] = evolution.get_fase(sorteo) if evolution else "desconocida"
    ...
    try:
        ...
        if len(numeros) >= 6:
            prediccion = {
                ...
                'n4': numeros[3], 'n5': numeros[4], 'plus': numeros[5] if len(numeros) > 5 else 0
            }
        
        u_score_tentativo = 50.0
        if 'OPTIMIZER' in intervenciones:
            confianza_match = re.search(r'confianza[:\s]*(\d+)', intervenciones['OPTIMIZER'], re.IGNORECASE)
            if confianza_match:
        ...
        validation_store[validation_id].update({
            ...
            "contradiccion_por_agente": resultado_validacion.get('contradiccion_por_agente', {}),
            "consenso_valido": puede_avanzar,
            "resultado_revelado": False,
        })
        
        logger.info("Validación %s completada para sorteo %s (contradicción: %.1f%%)", 
                    validation_id, sorteo, contradiccion)
...
async def get_dynamic_metrics() -> dict:
    ...
    v19_status_raw = get_v19_status() if hasattr(evolution, "_state") else {"congelado": False, "razon": ""}
...
async def start_validation(
    ...
    logger.info("Validación ciega %s iniciada para sorteo %s (fase: %s)", validation_id, sorteo_actual, fase)
    return {
...
async def reveal_validation_result(
    ...
    if validation.get("status") != "complete":
        raise HTTPException(status_code=400, detail="La validación aún no ha completado la predicción")
    ...
    if len(prediccion_numeros) >= 6:
        prediccion_dict = {
            'n1': prediccion_numeros[0], 'n2': prediccion_numeros[1],
            'n3': prediccion_numeros[2], 'n4': prediccion_numeros[3],
            'n5': prediccion_numeros[4], 'plus': prediccion_numeros[5]
        }
    
    resultado_dict = {
        'n1': resultado_numeros[0], 'n2': resultado_numeros[1],
        'n3': resultado_numeros[2], 'n4': resultado_numeros[3],
        'n5': resultado_numeros[4], 'plus': resultado_numeros[5] if len(resultado_numeros) > 5 else 0
    }
    
    acuerdo_real = calcular_acuerdo_real(prediccion_dict, resultado_dict)
    u_score_real = calcular_u_score(prediccion_dict, resultado_dict, confianza_declarada=validation.get('confianza', None))
    ...
    with get_db() as conn:
        ...
        if row:
            ...
            guardar_metrica_acumulada(sorteo, {
    ...
    logger.info("Resultado revelado para validación %s: %d aciertos, acuerdo real: %.1f%%, U-Score real: %.1f", 
                validation_id, aciertos_declarados, acuerdo_real, u_score_real)
...
async def list_debates() -> dict:
    return {
        "debates": [
            {"debate_id": k, "status": v.get("status")}
            for k, v in debate_store.items()
        ]
    }
...
Respondé de forma clara, directa y útil."""
    ...
    try:
        ...
        if result.debate and result.debate.synthesis:
            ...
            for step in result.steps:
                if step.agent_name == best_name and step.success:
                    response_text = step.result.get("output", str(step.result)) if isinstance(step.result, dict) else str(step.result)
                    break
            else:
        ...
        return {
            ...
            "agent_used": result.scores_summary.get('best_agent', 'unknown') if hasattr(result, 'scores_summary') else 'unknown'
        }
