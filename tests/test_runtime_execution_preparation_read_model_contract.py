...
DOC = ROOT / "docs" / "RUNTIME_EXECUTION_PREPARATION_READ_MODEL_CONTRACT.md"
MODULE_PATH = ROOT / "core" / "runtime_execution_preparation_read_model.py"


def _metadata(**overrides):
    data = {
        "read_model_reason": "unit test",
        "read_model_scope": "contract",
        "created_by": "pytest",
        "source": "tests",
        "tags": ("read_model", "safe"),
        "notes": ("read only",),
        "package_ref": "pkg_1",
...
def test_dataclasses_are_frozen():
    for name in [
        "RuntimeExecutionPreparationReadModelPolicy",
        ...
        cls = getattr(read_model_contract, name)
        assert dataclasses.is_dataclass(cls)
        assert cls.__dataclass_params__.frozen is True


def test_enums_allowed_forbidden_readiness_visibility_and_capabilities():
    for status in [
        ...
        assert status in read_model_contract.ALLOWED_STATUSES
    for status in [
        "read_model_active",
        "read_model_running",
        ...
        assert status in read_model_contract.FORBIDDEN_STATUSES
    assert read_model_contract.ALLOWED_READINESS == (
    ...
    for readiness in [
        ...
        assert readiness in read_model_contract.FORBIDDEN_READINESS
    for visibility in ["master_panel_view", "user_panel_view", "internal_audit_view", "internal_only", "blocked"]:
        assert visibility in [item.value for item in read_model_contract.RuntimeExecutionPreparationReadModelVisibility]
    for capability in [
        "runtime_execution",
        "runtime_activation",
        "dry_run_execution",
        ...
        assert capability in read_model_contract.BLOCKED_CAPABILITIES
...
def test_metadata_safe_values_survive_and_dangerous_values_are_blocked():
    metadata = read_model_contract.sanitize_runtime_execution_preparation_read_model_metadata(
        {
            "read_model_reason": "safe",
            "package_ref": "pkg",
            "contract_ref": "contract",
            "api_key": "SECRET_VALUE",
            "raw_prompt": "RAW_PROMPT_VALUE",
            "model_response": "MODEL_RESPONSE_VALUE",
            "master_panel_internal_capability": "MASTER_VALUE",
            "permission_bypass": "BYPASS_VALUE",
        }
    )
    dumped = json.dumps(read_model_contract.runtime_execution_preparation_read_model_to_dict(metadata), sort_keys=True)
    assert metadata.read_model_reason == "safe"
    assert metadata.package_ref == "pkg"
    assert metadata.contract_ref == "contract"
    ...
    for value in ["SECRET_VALUE", "RAW_PROMPT_VALUE", "MODEL_RESPONSE_VALUE", "MASTER_VALUE", "BYPASS_VALUE"]:
...
def test_policy_default_deny_and_operational_policy_fails():
    ...
    unsafe = dataclasses.replace(policy, api_enabled=True)
    result = _validation(policy=unsafe)
    assert result.is_valid is False
    assert "operational_policy_flag_enabled:api_enabled" in result.policy_violations
...
def test_source_ref_complete_and_incomplete_detection():
    ...
    incomplete = _source_ref(package_id="", preparation_id="", intent_ref="", source_package_ref="", source_contract_ref="")
...
def test_forbidden_readiness_status_metadata_policy_and_views_fail():
    ...
    assert "read_model_active" in status.forbidden_status_detected
    ...
    assert "operational_policy_flag_enabled:writes_enabled" in policy.policy_violations
    ...
    user = _validation(model, views=(master, unsafe_user, audit))
    assert user.is_valid is False
    assert any("user_panel_view_contains_forbidden_fragment" in item for item in user.view_violations)


def test_views_do_not_expose_raw_or_sensitive_values():
    model = _read_model(metadata=_metadata(api_key="SECRET_VALUE", raw_prompt="RAW_PROMPT_VALUE"))
    master, user, audit = _views(model)
    for view in [master, user]:
        dumped = json.dumps(read_model_contract.runtime_execution_preparation_read_model_to_dict(view), sort_keys=True)
        for token in [
            "SECRET_VALUE",
            "RAW_PROMPT_VALUE",
            "raw_prompt",
    ...
    audit_dump = json.dumps(read_model_contract.runtime_execution_preparation_read_model_to_dict(audit), sort_keys=True)
    for token in ["SECRET_VALUE", "RAW_PROMPT_VALUE", "authorization", "bearer", "personal_data_unsanitized"]:
        assert token not in audit_dump
    assert "raw_prompt" in audit.blocked_keys
    user_dump = json.dumps(read_model_contract.runtime_execution_preparation_read_model_to_dict(user), sort_keys=True)
    for token in ["metadata", "technical_refs", "master_panel", "admin", "security_internal", "permission_internal"]:
...
def test_decision_positive_and_negative_never_allow_operational_axes():
    ...
    for case in cases:
        decision = read_model_contract.decide_runtime_execution_preparation_read_model(case)
...
def test_to_dict_snapshots_json_safe_and_deterministic():
    ...
    assert first == second
    json.dumps(read_model_contract.runtime_execution_preparation_read_model_to_dict(snapshot), sort_keys=True)
...
def test_obliteratus_excluded_from_read_model_roles():
    ...
    for statement in read_model_contract.OBLITERATUS_EXCLUSION_STATEMENTS:
