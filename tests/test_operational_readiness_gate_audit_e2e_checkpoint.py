...
DOC = ROOT / "docs" / "OPERATIONAL_READINESS_GATE_AUDIT_E2E_CHECKPOINT.md"
...
def test_operational_readiness_gate_audit_e2e_checkpoint_exists():
    text = _text()

    assert DOC.exists()
    assert "OPERATIONAL_READINESS_GATE_AUDIT_E2E_PASSED" in text
...
def test_checkpoint_confirms_no_real_gate_or_runtime_boundaries():
    text = _text()

    for boundary in [
        "No se activo operational readiness gate real",
        "runtime",
        "attempt factory",
        "store writes",
        "lifecycle writes",
