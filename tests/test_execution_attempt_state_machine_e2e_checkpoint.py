...
CHECKPOINT_DOC = ROOT / "docs" / "EXECUTION_ATTEMPT_STATE_MACHINE_E2E_CHECKPOINT.md"
...
def test_state_machine_e2e_checkpoint_exists_and_declares_chain():
    text = _text()

    assert CHECKPOINT_DOC.exists()
    assert "EXECUTION_ATTEMPT_STATE_MACHINE_E2E_PASSED" in text
    for prompt in [
...
def test_state_machine_e2e_checkpoint_contains_verdicts_and_readiness():
    text = _text()
...
def test_runtime_store_lifecycle_result_and_worker_boundaries_disabled():
...
def test_state_machine_e2e_checkpoint_has_no_contradictory_enabled_states():
    text = _text()
