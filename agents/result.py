...
def calcular_contradiccion_real(intervenciones: dict) -> float:
    ...
    def get_key_words(text: str) -> set:
        text = text.lower()
        stop_words = {'el', 'la', 'los', 'las', 'un', 'una', 'unos', 'unas', 'y', 'o', 'pero', 'que', 'es', 'son', 
                      'está', 'están', 'por', 'para', 'con', 'sin', 'sobre', 'tras', 'durante', 'mediante',
                      'the', 'a', 'an', 'and', 'of', 'to', 'in', 'for', 'on', 'with', 'by', 'at', 'from',
                      'this', 'that', 'these', 'those', 'is', 'are', 'was', 'were', 'be', 'been', 'being',
                      'del', 'una', 'unas', 'unos', 'ante', 'bajo', 'cabe', 'contra', 'entre'}
        words = set()
...
def calcular_contradiccion_por_pares(intervenciones: dict) -> dict:
    ...
    def get_words(text: str) -> set:
        text = text.lower()
        stop_words = {'el', 'la', 'los', 'las', 'un', 'una', 'y', 'o', 'pero', 'que', 'es', 'son', 'con', 'sin', 'por', 'para'}
        words = set()
        for word in text.split():
            word = word.strip('.,!?;:()[]{}"\'')
...
def calcular_u_score(prediccion: dict, resultado: dict = None, confianza_declarada: float = None) -> float:
    """
...
def validar_consenso(intervenciones: dict, contradiccion_minima: float = 20.0) -> dict:
    ...
    optimizer_text = intervenciones.get('OPTIMIZER', '')
    optimizer_invalido = 'INVÁLIDO' in optimizer_text.upper() or 'CONSENSO SUSPECHOSO' in optimizer_text.upper()
