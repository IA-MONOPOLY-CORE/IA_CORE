...
async def main():
    ...
    try:
        ...
        result = await supervisor.orchestrate_async(
            task=TASK, agent_names=GENERIC_AGENTS, mode=ExecutionMode.DEBATE
        )
        ...
        assert all(agent.startswith("generic_") for agent in used_agent_ids), (
            "No se usaron agentes genéricos!"
        )
